...
class User extends CI_Controller {
    ...
    public function register() {
        // loding the default library in codeigniter
        ...
        if ($this->form_validation->run()) {
            ...
            $data['CustomerMessage'] = $this->Model_User->getRegister
                    ($firstname, $lastname, $username, $password, $phonenumber, $address, $email);
    ...
    public function login() {
        ...
        if ($userID) {
            ...
            if ($userID == 1) {
                ...
                window.location.href='Home/dashboard'; </script>";
            }
        } else {
    ...
    public function selectUser() {
        //loding model
        $this->load->model('Model_User');
        //selecting data from model
        $data['message'] = $this->Model_User->selectUser();
        //passing message to view through variable $data
